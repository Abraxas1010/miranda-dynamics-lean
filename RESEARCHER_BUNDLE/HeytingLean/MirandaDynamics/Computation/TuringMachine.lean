...
/-- `stepN` agrees with Mathlib's `Function.iterate` for the step function. -/
theorem stepN_eq_iterate (M : DetSys Cfg) (n : Nat) (c : Cfg) :
    stepN M n c = (M.step)^[n] c := by
  induction n generalizing c with
  | zero =>
    rfl
  | succ n ih =>
    simp [DetSys.stepN, Function.iterate_succ_apply, ih]
...
/-- `HaltSys.stepN` agrees with `Function.iterate` for the step function. -/
theorem stepN_eq_iterate (M : HaltSys Cfg) (n : Nat) (c : Cfg) :
    M.stepN n c = (M.step)^[n] c := by
  simpa [HaltSys.stepN] using (DetSys.stepN_eq_iterate (M := M.toDetSys) n c)
