...
/-! ### Uniform bounds: `I_k ⊆ [0,1]` -/

theorem headScale_le_one_third (k : ℤ) : headScale k ≤ (1 / 3 : ℝ) := by
  -- `headScale k = (3^(1+|k|))⁻¹ ≤ (3^1)⁻¹ = 1/3`.
  have hbase : (3 : ℝ) ^ (1 : ℕ) ≤ (3 : ℝ) ^ (1 + Int.natAbs k) := by
    have : (1 : ℕ) ≤ 1 + Int.natAbs k := Nat.le_add_right 1 (Int.natAbs k)
    exact pow_le_pow_right₀ (by norm_num) this
  have hpos : 0 < (3 : ℝ) ^ (1 : ℕ) := by positivity
  have hinv : ((3 : ℝ) ^ (1 + Int.natAbs k))⁻¹ ≤ ((3 : ℝ) ^ (1 : ℕ))⁻¹ := by
    simpa [one_div] using one_div_le_one_div_of_le hpos hbase
  simpa [headScale] using hinv

theorem headLeft_nonneg (k : ℤ) : 0 ≤ headLeft k := by
  by_cases hk : k < 0
  · simp [headLeft, hk, le_of_lt (headScale_pos k)]
...
theorem headRight_le_one (k : ℤ) : headRight k ≤ 1 := by
  by_cases hk : k < 0
  · have hs : headScale k ≤ (1 / 3 : ℝ) := headScale_le_one_third k
    have : 2 * headScale k ≤ (2 / 3 : ℝ) := by nlinarith
    have : headRight k ≤ (2 / 3 : ℝ) := by simpa [headRight, hk] using this
    exact le_trans this (by norm_num)
  · have hspos : 0 ≤ headScale k := le_of_lt (headScale_pos k)
    -- `headRight k = 1 - headScale k ≤ 1`.
    simpa [headRight, hk, sub_eq_add_neg] using (sub_le_self (a := (1 : ℝ)) hspos)

theorem headInterval_subset_Icc (k : ℤ) : headInterval k ⊆ Set.Icc (0 : ℝ) 1 := by
  intro x hx
  refine ⟨?_, ?_⟩
  · exact le_trans (headLeft_nonneg k) hx.1
  · exact le_trans hx.2 (headRight_le_one k)
...
/-- The affine “head shift” map on the head-coordinate intervals (paper Lemma 1). -/
noncomputable def headShift (k : ℤ) (z : ℝ) : ℝ :=
  if k < 0 then 3 * z else z / 3 + 2 / 3

theorem headShift_tau (k : ℤ) (x : ℝ) : headShift k (tau k x) = tau (k + 1) x := by
  simp [headShift, tau_shift_law]

theorem tau_surj_headInterval (k : ℤ) {z : ℝ} (hz : z ∈ headInterval k) :
    ∃ x : ℝ, x ∈ Set.Icc (0 : ℝ) 1 ∧ tau k x = z := by
  have hkpos : 0 < headScale k := headScale_pos k
  have hk0 : 0 ≤ headScale k := le_of_lt hkpos
  by_cases hk : k < 0
  · -- negative branch: `τ_k(x) = s*(1+x)` maps `[0,1]` onto `[s,2s]`.
    refine ⟨z / headScale k - 1, ?_, ?_⟩
    · have hz' : z ∈ Set.Icc (headScale k) (2 * headScale k) := by
        simpa [headInterval, headLeft, headRight, hk] using hz
      have hsne : headScale k ≠ 0 := ne_of_gt hkpos
      constructor
      · -- `0 ≤ z/s - 1` since `s ≤ z`.
        have : (1 : ℝ) ≤ z / headScale k := by
          have hzL : headScale k ≤ z := hz'.1
          have hmul :
              headScale k * (headScale k)⁻¹ ≤ z * (headScale k)⁻¹ :=
            mul_le_mul_of_nonneg_right hzL (inv_nonneg.2 hk0)
          simpa [div_eq_mul_inv, hsne, mul_assoc, mul_left_comm, mul_comm] using hmul
        linarith
      · -- `z/s - 1 ≤ 1` since `z ≤ 2s`.
        have : z / headScale k ≤ 2 := by
          have hzU : z ≤ 2 * headScale k := hz'.2
          have hmul :
              z * (headScale k)⁻¹ ≤ (2 * headScale k) * (headScale k)⁻¹ :=
            mul_le_mul_of_nonneg_right hzU (inv_nonneg.2 hk0)
          simpa [div_eq_mul_inv, hsne, mul_assoc, mul_left_comm, mul_comm] using hmul
        linarith
    · -- check that `τ_k(x) = z`
      have hsne : headScale k ≠ 0 := ne_of_gt hkpos
      calc
        tau k (z / headScale k - 1) = headScale k * (1 + (z / headScale k - 1)) := by
          simp [tau, hk]
        _ = headScale k * (z / headScale k) := by
          ring_nf
        _ = z := by
          -- `s * (z/s) = z` for `s ≠ 0`.
          have : headScale k * (z * (headScale k)⁻¹) = z := by
            calc
              headScale k * (z * (headScale k)⁻¹)
                  = (headScale k * z) * (headScale k)⁻¹ := by
                      simp [mul_assoc]
              _ = (z * headScale k) * (headScale k)⁻¹ := by
                      simp [mul_comm]
              _ = z * (headScale k * (headScale k)⁻¹) := by
                      simp [mul_assoc]
              _ = z := by
  ...
  · -- nonnegative branch: `τ_k(x) = 1 + s*(-2 + x)` maps `[0,1]` onto `[1-2s, 1-s]`.
    refine ⟨(z - 1) / headScale k + 2, ?_, ?_⟩
    · have hz' : z ∈ Set.Icc (1 - 2 * headScale k) (1 - headScale k) := by
        simpa [headInterval, headLeft, headRight, hk] using hz
      have hsne : headScale k ≠ 0 := ne_of_gt hkpos
      constructor
      · -- lower bound: `0 ≤ (z-1)/s + 2`
        have : (-2 : ℝ) ≤ (z - 1) / headScale k := by
          have hzL : -2 * headScale k ≤ z - 1 := by linarith [hz'.1]
          have hmul :
              (-2 * headScale k) * (headScale k)⁻¹ ≤ (z - 1) * (headScale k)⁻¹ :=
            mul_le_mul_of_nonneg_right hzL (inv_nonneg.2 hk0)
          simpa [div_eq_mul_inv, hsne, mul_assoc, mul_left_comm, mul_comm] using hmul
        linarith
      · -- upper bound: `(z-1)/s + 2 ≤ 1`
        have : (z - 1) / headScale k ≤ (-1 : ℝ) := by
          have hzU : z - 1 ≤ -headScale k := by linarith [hz'.2]
          have hmul :
              (z - 1) * (headScale k)⁻¹ ≤ (-headScale k) * (headScale k)⁻¹ :=
            mul_le_mul_of_nonneg_right hzU (inv_nonneg.2 hk0)
          simpa [div_eq_mul_inv, hsne, mul_assoc, mul_left_comm, mul_comm] using hmul
        linarith
    · have hsne : headScale k ≠ 0 := ne_of_gt hkpos
      calc
        tau k ((z - 1) / headScale k + 2) = 1 + headScale k * (-2 + ((z - 1) / headScale k + 2)) := by
          simp [tau, hk]
        _ = 1 + headScale k * ((z - 1) / headScale k) := by
          ring_nf
        _ = z := by
          have hmul : headScale k * ((z - 1) * (headScale k)⁻¹) = (z - 1) := by
            calc
              headScale k * ((z - 1) * (headScale k)⁻¹)
                  = (headScale k * (z - 1)) * (headScale k)⁻¹ := by
                      simp [mul_assoc]
              _ = ((z - 1) * headScale k) * (headScale k)⁻¹ := by
                      simp [mul_comm]
              _ = (z - 1) * (headScale k * (headScale k)⁻¹) := by
                      simp [mul_assoc]
              _ = (z - 1) := by
                      simp [hsne]
          have : 1 + headScale k * ((z - 1) * (headScale k)⁻¹) = z := by
            -- `1 + (z - 1) = z`.
            calc
              1 + headScale k * ((z - 1) * (headScale k)⁻¹) = 1 + (z - 1) := by simp [hmul]
              _ = z := by ring_nf
          simpa [div_eq_mul_inv] using this

theorem headShift_mem_headInterval_succ (k : ℤ) {z : ℝ} (hz : z ∈ headInterval k) :
    headShift k z ∈ headInterval (k + 1) := by
  rcases tau_surj_headInterval k hz with ⟨x, hx, rfl⟩
  simpa [headShift_tau] using tau_mem_headInterval (k := k + 1) (x := x) hx

/-- `tau k` is injective (affine with nonzero slope). -/
theorem tau_injective (k : ℤ) : Function.Injective (tau k) := by
  ...
  · have h' : headScale k * (1 + x) = headScale k * (1 + y) := by
    ...
    have h'' : (1 : ℝ) + x = 1 + y := (mul_left_cancel₀ hs0) h'
    linarith
  · have h' : 1 + headScale k * (-2 + x) = 1 + headScale k * (-2 + y) := by
      simpa [tau, hk] using hxy
    have h'' : headScale k * (-2 + x) = headScale k * (-2 + y) :=
      add_left_cancel h'
    have h''' : (-2 : ℝ) + x = -2 + y := (mul_left_cancel₀ hs0) h''
    linarith
...
theorem encodeWithHead_injective_tape (k : ℤ) : Function.Injective fun t : Tape => encodeWithHead t k := by
  intro t₁ t₂ h
  have hEnc : encodeTape t₁ = encodeTape t₂ := by
    apply tau_injective (k := k)
    simpa [encodeWithHead] using h
  exact encodeTape_injective hEnc

theorem encodeWithHead_injective_pair :
    Function.Injective fun p : Tape × ℤ => encodeWithHead p.1 p.2 := by
  rintro ⟨t₁, k₁⟩ ⟨t₂, k₂⟩ h
  by_cases hk : k₁ = k₂
  · cases hk
    have ht : t₁ = t₂ :=
      encodeWithHead_injective_tape (k := k₁) (by simpa using h)
    exact Prod.ext ht rfl
  · have hk1 : encodeWithHead t₁ k₁ ∈ headInterval k₁ :=
      tau_mem_headInterval (k := k₁) (x := encodeTape t₁) (encodeTape_mem_Icc t₁)
    have hk2 : encodeWithHead t₂ k₂ ∈ headInterval k₂ :=
      tau_mem_headInterval (k := k₂) (x := encodeTape t₂) (encodeTape_mem_Icc t₂)
    have hk2' : encodeWithHead t₁ k₁ ∈ headInterval k₂ := by
      simpa [h] using hk2
    have hdis : Disjoint (headInterval k₁) (headInterval k₂) :=
      headInterval_disjoint k₁ k₂ hk
    exact False.elim ((Set.disjoint_left.1 hdis) hk1 hk2')
